...
def build_network(inputs, dec_inputs,char2numY,n_channels=10,input_depth=280,num_units=128,max_time=10,bidirectional=False):
    ...
    data_output_embed = tf.nn.embedding_lookup(output_embedding, dec_inputs)
        ...
        # 编码器
    with tf.variable_scope("encoding") as encoding_scope: # 定义编码器的变量作用域
        if not bidirectional:
            # 使用GRU单元的单向GRU
            gru_enc = tf.contrib.rnn.GRUCell(num_units)
            _, last_state = tf.nn.dynamic_rnn(gru_enc, inputs=data_input_embed, dtype=tf.float32)
        else:
            # 使用双向GRU架构
            enc_fw_cell = tf.contrib.rnn.GRUCell(num_units)
            enc_bw_cell = tf.contrib.rnn.GRUCell(num_units)
            ...
            ((enc_fw_out, enc_bw_out), (enc_fw_final, enc_bw_final)) = tf.nn.bidirectional_dynamic_rnn(
                ...
                dtype=tf.float32)
            last_state = tf.concat((enc_fw_final, enc_bw_final), 1)
    
    # 解码器
    with tf.variable_scope("decoding") as decoding_scope:
        if not bidirectional:
            gru_dec = tf.contrib.rnn.GRUCell(num_units)
        else:
            gru_dec = tf.contrib.rnn.GRUCell(2 * num_units)
    
        dec_outputs, _ = tf.nn.dynamic_rnn(gru_dec, inputs=data_output_embed, initial_state=last_state)
...
def main():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='checkpoints-2_1')
...
def run_program(args):
    ...
    # ratio={0:nums[0],1:n_oversampling+40000,2:n_oversampling+40000}
    ...
    def test_model():
        ...
        sum_test_conf = np.sum(np.array(sum_test_conf, dtype=np.int32), axis=0)
